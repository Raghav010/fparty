...
dbc=db.cursor()
dbc.execute("CREATE TABLE if not exists UserData (phone_number varchar(20) primary key,name varchar(200))")
dbc.execute("create table if not exists foodata (partyid integer primary key,food varchar(150),place varchar(150), userphn varchar(20), price_req integer)")
...
@app.route('/joinparty/<partyid>', methods=["POST","GET"])
def joinParty(partyid):
    if(session.get('logged',-1)!=True):
        ...
        if session.request = "GET":
            ...
            dbc.execute("select * from foodata where partyid = ?", (partyid, ))
            sessionData = dbc.fetchone()
            
            return render_template("party.html", partyFood = sessionData[1]
                                                 partyPlace = sessionData[2]
                                                 partyhone = sessionData[3])
